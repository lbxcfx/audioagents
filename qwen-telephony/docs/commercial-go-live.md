...
- [ ] LiveKit Egress 服务和合规对象存储已部署；验证录音先于业务语音启动、双声道文件可回放、Egress 失败会按强制录音策略终止呼叫；若配置了录音告知，验证其在业务开场前完整播放。
...
- [ ] 如录音/转写，告知方式或其他合法依据、敏感字段脱敏、加密、访问控制和保存期限已经法务批准；空告知配置不代表系统自动取得录音授权。
