...
服务 Token 以文件挂载。Dispatcher 和 Agent 每次请求都重新读取文件，因此 Secret Controller 可以无重启轮换短期 Token。两类工作负载必须分别使用 `cloud-parity-dispatcher-service-token` 与 `cloud-parity-agent-service-token` 对应的独立 Client Credentials 主体，避免共享限流桶和轮换故障域；创建项目后分别以 `worker` 角色加入项目。不要给人类账号分配该角色，也不要让 Agent/Dispatcher 复用 owner、admin 或 member 令牌。`worker` 仅具有呼叫租约执行与会话写入权限。

不要把 `cloud-parity-secrets` 挂载给工作节点：它包含数据库凭据、Fernet 主密钥和号码哈希密钥。Dispatcher 只挂载 `cloud-parity-dispatcher-secrets`，Agent 只挂载 `cloud-parity-agent-secrets`。三个 Secret 中重复出现的值由 External Secrets/KMS 从同一受控源同步，但权限策略必须按工作负载分别授权。
