...
4. Call `audioagent_submit_outbound_task` only after explicit confirmation and pass `confirmed=true`. Never infer confirmation from the original task request.
...
7. Cancel only after a separate explicit cancellation confirmation.
